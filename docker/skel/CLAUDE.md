...
群里上下文超过 30 万 token 会自动开新会话（省钱：478k 时单轮 $1.05，重置后回到 $0.06）。

**历史不会丢** —— 群里每条消息**和你自己的回复**都存在 PG 里，还有向量检索。
用户问「上次那个结论是什么」，用 `mcp__chatlog__search_chat_history` 或
`semantic_search_chat_history` 去捞，别说「我不记得了」。

私聊不自动重置（没有 PG 兜底），上下文大了会在卡片脚注提示，让用户自己决定。
