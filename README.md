...
- **收发图片和文件**：收到的下载到工作目录给 agent 读；agent 生成的图表 / 数据文件
...
**每个任务有自己的会话**（`${chatId}#task<id>`），而且每次从头跑（`fresh`）。
早先和用户的聊天共用一个会话，每次触发都往里 append —— 一个每小时的喝水提醒
跑了 101 次，会话文件涨到 18MB，之后每轮**普通对话**都要把这堆重发一遍，
单轮成本从 $0.25 飙到 $4.49，用户的上下文里还全是「💧 该喝水啦」。
上一次的简报对这一次也没有参考价值，留着只是负担。
...
（`on_` 开头 + 32 位十六进制，如 `on_4c87c9ce46ad3ae81c4389ec484c8b76`）。
注意 union_id 是**租户内**稳定，跨租户不通：同一个人在两个租户下是两个值。

**不在白名单的人私聊时，bot 会回一句并附上他自己的 union_id**，让他直接转给管理员，
省掉「你去翻服务器日志」这一步。每人只提示一次 —— 任何人都能私聊 bot，
每条都回就成了免费回声墙。加进 `users.json` 即刻生效，不用重启。
...
这个架构下**一条消息 = 一个新 CLI 进程**，prompt cache 跨不到下一轮。实测：

```
缓存写 418,824   ← 每轮开头，全量重写（$18.75/M）
缓存读 436,035   ← 轮内后续，命中（$1.50/M，1/12.5）
缓存写 424,334   ← 下一轮，又全量重写
```

失效次数 ≈ 轮次数。所以**上下文有多大，每轮就重付多少**：

| 上下文 | 每轮 |
|---|---|
| 425k tok（被定时任务撑爆的会话） | ≈ $7.97 |
| 30k tok（正常会话） | ≈ $0.56 |
| 3k tok（空会话地板） | ≈ $0.06 |

**自动压缩指望不上。** 它按上下文窗口占比触发（~90%），而窗口有 100 万 token，
意味着要涨到 90 万才动手 —— 那时每轮光读缓存就 $1.35。实测五个实例、几百轮对话，
一次都没触发过（`compact` 记录 0 条）。它的设计目标是「别让上下文溢出」，不是省钱。

所以桥接自己盯着：超过 `CONTEXT_WARN_TOKENS`（默认 15 万）就在卡片脚注提示
「上下文 436k ≈ $0.65/轮，建议 /compact 或 /new」，`/status` 里也能看到。
**只提示不自动压** —— 压缩会丢细节，该不该压用户比程序清楚。

能压的只有上下文大小：

- **定时任务独立会话** —— 最大的一笔，见上面「定时任务」一节
- **关掉用不上的内置工具**（`OFF_TOOLS`）—— 工具定义每轮都重付。
  砍掉 11 个会话级/交互式/用不到的工具 + 精简系统提示，
  实测地板从 7,339 → 3,024 tok（-59%）
- **群聊上下文只塞 10 条**，更早的让 agent 自己检索
- 换话题时 `/new`

`mcp.json` 里的 `lark` 用 `preset.default`（20+ 工具），只在群聊加载 ——
用过的分布很散（群成员 / 云文档 / 多维表格都有），砍了会真丢功能，保留。
