...
- **长期记忆**：群聊和私聊的消息都实时落库 + 向量化，支持关键词 / 语义检索
...
存档时机两边**故意不一样**：群聊在鉴权**之前**存（群里所有人的发言都是上下文，
不该因为发言人不在白名单就丢掉），私聊在鉴权**之后**存（未授权的人不该在库里留记录）。

⚠️ 私聊内容会落到 PG。自用无所谓，但给同事用的 bot 等于把他们的私聊存进你的库，
不想要就把 `plugins/chatlog.mts` 的 `scope` 改回 `'group'`，并把 `index.mts` 里
鉴权之后那句 `archive()` 去掉。
